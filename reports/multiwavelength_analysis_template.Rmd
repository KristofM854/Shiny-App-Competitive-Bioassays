...
for (wl in wavelengths) {
  ...
  if (file.exists(csv_file)) {
    ...
    if (file.exists(stats_file)) {
      ...
      if (!is.null(stats)) {
        wl_info$r_squared <- tryCatch(as.numeric(stats$r_squared), error = function(e) NA_real_)
        wl_info$rmse <- tryCatch(as.numeric(stats$rmse), error = function(e) NA_real_)
        wl_info$ic50 <- tryCatch(as.numeric(stats$ic50), error = function(e) NA_real_)
        wl_info$mean_cv <- tryCatch(as.numeric(stats$mean_sample_cv), error = function(e) NA_real_)
      }
    }
...
if (length(summary_data) > 0) {
  summary_df <- data.frame(
    ...
    R2 = sapply(summary_data, function(x) {
      val <- x$r_squared; if (is.null(val) || !is.numeric(val) || is.na(val)) "\u2014" else format(round(val, 4), nsmall = 4)
    }),
    RMSE = sapply(summary_data, function(x) {
      val <- x$rmse; if (is.null(val) || !is.numeric(val) || is.na(val)) "\u2014" else format(round(val, 3), nsmall = 3)
    }),
    IC50 = sapply(summary_data, function(x) {
      val <- x$ic50; if (is.null(val) || !is.numeric(val) || is.na(val)) "\u2014" else format(round(val, 1), nsmall = 1, big.mark = ",")
    }),
    Standards = sapply(summary_data, function(x) if (is.null(x$n_standards)) NA else x$n_standards),
    Samples = sapply(summary_data, function(x) if (is.null(x$n_samples)) NA else x$n_samples),
    Quantified = sapply(summary_data, function(x) if (is.null(x$n_quantified)) NA else x$n_quantified),
    Mean_CV = sapply(summary_data, function(x) {
      val <- x$mean_cv; if (is.null(val) || !is.numeric(val) || is.na(val)) "\u2014" else paste0(format(round(val, 1), nsmall = 1), "%")
    }),
  ...
  rmse_vals <- sapply(summary_data, function(x) {
    v <- x$rmse
    if (is.null(v) || !is.numeric(v) || is.na(v)) NA_real_ else v
  })

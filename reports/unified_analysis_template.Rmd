...
tryCatch({
  if (isTRUE(is_elisa) && exists("tissue_weights") && !is.null(tissue_weights) && length(tissue_weights) > 0) {
    ...
    cat("$$C_{tissue} = \\frac{C_{extract} \\times V_{extraction}}{m_{tissue}}$$\n\n")
    cat("Where:\n\n")
    cat("- $C_{extract}$ = concentration in extract (pg/mL from curve)\n")
    cat("- $V_{extraction}$ = extraction volume (mL)\n")
    cat("- $m_{tissue}$ = tissue mass (g)\n")

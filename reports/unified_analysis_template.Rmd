...
tryCatch({
  ...
  exec_analyte <- assay_config$analyte %||% assay_config$toxin_standard_label %||% "Unknown"

  # Standards info
  n_std_concs <- length(unique(
    data_long$StandardConc[data_long$SampleType == "Standard" & !is.na(data_long$StandardConc)]
  ))
  n_hv <- nrow(identify_high_variability_standards(data_long, cv_limit = cv_limit))
  std_note <- sprintf("<strong>Standards:</strong> %d concentration levels", n_std_concs)
  if (n_hv > 0) std_note <- paste0(std_note, sprintf(" (%d excluded for high CV)", n_hv))
  ...
  lines <- c(
    ...
    sprintf("<strong>Assay:</strong> %s &mdash; %s<br>", exec_assay_type, exec_analyte),
  ...
  emit_styled_block(lines,
    html_style = "border: 2px solid #2196F3; padding: 15px; border-radius: 8px; background: #F5F9FF; margin: 10px 0 20px 0;")
}, error = function(e) {
...
# If the user picked "Auto", fit an unweighted LL.4 first, run the
# heteroscedasticity diagnostic, and rewrite selected_weights to the single
# chosen weight. auto_weighting_result is consumed by the weighting-
# suitability chunk further down so the report explains the auto decision.
...
if ("auto" %in% selected_weights) {
  ...
  if (is.null(initial_fit)) {
    # Initial unweighted fit failed — default to 1/Y^2 without diagnostic.
    auto_weighting_result <- list(
      chosen_weight = "inv_y2",
      fallback = TRUE,
      test_name = NA_character_, statistic = NA_real_, p_value = NA_real_,
...
if (isTRUE(is_elisa)) {
  ...
  ec20 <- tryCatch({
    ed_result <- ED(model_fit, respLev = STATS_CONFIG$ec20_resp_level, type = "absolute", display = FALSE)
    as.numeric(ed_result[1, 1])
  }, error = function(e) NA_real_)
  ec80 <- tryCatch({
    ed_result <- ED(model_fit, respLev = STATS_CONFIG$ec80_resp_level, type = "absolute", display = FALSE)
    as.numeric(ed_result[1, 1])
  }, error = function(e) NA_real_)
...
if (!drc_failed_completely && exists("R2") && !is.na(R2)) {
  ...
  # Load QC profile for assay-specific thresholds
  qc_profile <- get_qc_profile(assay_config$assay_type %||% "rba")

  # Determine overall status using assay-specific R2 threshold
  ...
  if (R2 >= 0.99) {
    ...
    exec_warnings <- c(exec_warnings, sprintf("R\u00B2 below threshold (%.4f < %.2f)", R2, qc_profile$r2_threshold))
  }
  ...
  if (!is.null(primary_model_entry$convergence) && !primary_model_entry$convergence$se_available) {
    ...
    if (overall_status == "Pass") { overall_status <- "Qualified Pass"; status_icon <- "\U0001F7E1" }
  }
  if (!is.null(primary_model_entry$lack_of_fit) && !is.null(primary_model_entry$lack_of_fit$p_value) && primary_model_entry$lack_of_fit$p_value < 0.05) {
    exec_warnings <- c(exec_warnings, sprintf("Lack-of-fit (p = %.4f)", primary_model_entry$lack_of_fit$p_value))
    if (overall_status == "Pass") { overall_status <- "Qualified Pass"; status_icon <- "\U0001F7E1" }
  }
  if (nrow(high_var_standards) > 0) {
    exec_warnings <- c(exec_warnings, sprintf("%d standard level(s) excluded for high CV", nrow(high_var_standards)))
  }
  ...
  cat(sprintf("**Assay:** %s \u2014 %s | **Model:** %s\n\n",
              ...
              assay_config$analyte %||% assay_config$toxin_standard_label %||% "Unknown",
              weight_desc))
  cat(sprintf("**Curve fit:** R\u00B2 = %.4f | RMSE = %.3f\n\n", R2, RMSE))

  cat(sprintf("**Standards:** %d wells used (of %d unique concentrations)",
              nrow(standards_for_model), length(unique(standards_data$StandardConc))))
  if (nrow(high_var_standards) > 0) {
    cat(sprintf(" | %d excluded", nrow(high_var_standards)))
  }
  cat("\n\n")

  # LLOQ/ULOQ from quantifiable range (ec20/ec80)
  if (exists("ec20") && exists("ec80") && !is.na(ec20) && !is.na(ec80)) {
    conc_unit <- if (isTRUE(is_elisa)) assay_config$units %||% "pg/mL" else "mol/L"
    lloq_fmt <- if (isTRUE(is_elisa)) format(ec20, digits = 4, big.mark = ",") else format(ec20, scientific = TRUE, digits = 3)
    uloq_fmt <- if (isTRUE(is_elisa)) format(ec80, digits = 4, big.mark = ",") else format(ec80, scientific = TRUE, digits = 3)
    cat(sprintf("**Quantifiable range:** LLOQ = %s %s | ULOQ = %s %s\n\n", lloq_fmt, conc_unit, uloq_fmt, conc_unit))
  }

  # Number of quantified samples (will be 0 at this point since sample analysis is later)
...
tryCatch({
  ...
  # Define thresholds using assay-specific QC profile (M7)
  ...
  qc_items <- list(
    list(metric = tr("qc_r2", lang), value = round(R2, 4),
         status = if (R2 >= 0.99) "green" else if (R2 >= tl_qc_profile$r2_threshold) "amber" else "red"),
    list(metric = tr("qc_hill", lang), value = round(hill_slope, 3),
         status = if (hill_slope >= tl_hill_range[1] && hill_slope <= tl_hill_range[2]) "green"
                  else if (hill_slope >= tl_hill_range[1] * 0.6 && hill_slope <= tl_hill_range[2] * 1.3) "amber" else "red")
  )
  if (!is.na(max_cv) && is.finite(max_cv)) {
    qc_items[[length(qc_items) + 1]] <- list(
      metric = tr("qc_max_cv", lang), value = paste0(round(max_cv, 1), "%"),
      status = if (max_cv <= tl_qc_profile$cv_limit) "green" else if (max_cv <= tl_qc_profile$cv_limit * 2) "amber" else "red")
  }
  if (!is.na(mean_recovery) && is.finite(mean_recovery)) {
    ...
    qc_items[[length(qc_items) + 1]] <- list(
      ...
      status = if (mean_recovery >= rec_tight_lo && mean_recovery <= rec_tight_hi) "green"
               else if (mean_recovery >= tl_recovery[1] && mean_recovery <= tl_recovery[2]) "amber" else "red")
  }
...
if (nrow(samples_to_analyze) > 0) {
  ...
  # M6: Add interpolation vs extrapolation labels
  tryCatch({
  ...
  # Use compute_layered_uncertainty for each replicate group (Task A: HP5)
  ...
  layered_ci_list <- lapply(replicate_groups, function(rep_grp) {
    tryCatch({
      ...
      lu <- compute_layered_uncertainty(well_preds, grp_data$estimated_concentration,
                                         ci_method = ci_method)
      data.frame(
        Replicate = rep_grp,
  ...
  replicate_stats <- stats_data %>%
    ...
    # Join layered uncertainty columns (Task A: HP5)
    left_join(layered_ci_df, by = "Replicate") %>%
    mutate(
      # Task B (HP6): Preserve raw CI before truncation
      ci_lower_raw = ci_lower_combined,
      ci_upper_raw = ci_upper_combined,
      # Apply truncation for display
      ci_lower = pmax(ci_lower_combined, STATS_CONFIG$ci_truncation_floor),
      ci_upper = ci_upper_combined,
      # Track whether truncation occurred
      ci_truncated = !is.na(ci_lower_raw) & ci_lower_raw < STATS_CONFIG$ci_truncation_floor,
  ...
  replicate_range <- sample_results %>%
    ...
    summarise(
      ...
      # M6: Aggregate quantification_status per replicate group (majority vote)
  ...
  replicate_stats <- replicate_stats %>%
    left_join(replicate_range %>% dplyr::select(Replicate, mean_response, within_range, range_flag, quant_status), by = "Replicate")
  ...
  # Save replicate summary with layered uncertainty columns (Task A/B: HP5/HP6)
  replicate_summary <- replicate_stats %>%
    dplyr::select(
      ...
      any_of(c("mean_response", "within_range", "range_flag", "quant_status", "TissueWeight_mg", "concentration_pg_per_g"))
    ) %>%
    dplyr::rename(
...
tryCatch({
  if (isTRUE(is_elisa) && exists("tissue_weights") && !is.null(tissue_weights) && length(tissue_weights) > 0) {
    ...
    # Build per-sample conversion table
    tw_trace <- data.frame(
      Replicate = names(tissue_weights),
      stringsAsFactors = FALSE
    )
    ...
    tw_trace <- tw_trace[!is.na(tw_trace$TissueMass_mg) & tw_trace$TissueMass_mg > 0, , drop = FALSE]
    ...
    if (nrow(tw_trace) > 0) {
      ...
      display_tw <- tw_trace %>%
        dplyr::select(Replicate, TissueMass_mg, Extraction_uL, TissueMass_g, Extraction_mL)
      names(display_tw) <- c("Replicate Group", "Tissue Mass (mg)", "Extraction Vol. (uL)",
      ...
      # Worked example using the first sample that has a tissue mass and
      # a successfully quantified extract concentration. Lets a reviewer
      # reproduce the reported pg/g number from the printed inputs.
      if (exists("replicate_stats") && !is.null(replicate_stats) &&
        ...
        if (nrow(tw_example) > 0) {
          ...
          ex_trace <- tw_trace[tw_trace$Replicate == ex$Replicate, , drop = FALSE][1, , drop = FALSE]
...
tryCatch({
  ol_flags_for_audit <- if (exists("outlier_flags") && !is.null(outlier_flags) && nrow(outlier_flags) > 0) {
    outlier_flags
  } else {
    NULL
  }

  hv_for_audit <- if (exists("high_var_standards") && !is.null(high_var_standards) && nrow(high_var_standards) > 0) {
    high_var_standards
  } else {
    NULL
  }

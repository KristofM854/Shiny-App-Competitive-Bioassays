...
for (wt_key in selected_weights) {
  ...
  tryCatch({
    ...
    # Don't pass weights argument when NULL (unweighted) - some drc versions
    # treat explicit weights=NULL differently from omitting the argument
    if (isTRUE(is_elisa)) {
      if (is.null(wt_info$weights)) {
        fit <- drc::drm(fml, data = standards_for_model, fct = drc::LL.4(),
    ...
    all_models[[wt_key]] <- list(
      model = fit, label = wt_info$label,
      R2 = r2, RMSE = rmse, coefs = coefs
    )
  }, error = function(e) {
    warning("Model fitting failed for weight '", wt_key, "': ", e$message)
  })
}

# Primary model = first successfully fitted weighting (used for sample calculation)
if (length(all_models) == 0) {
  stop("No dose-response models could be fitted. Check your standard curve data.")
}
...
# Reuse write_json_safe if already defined (from global.R), otherwise define it
if (!exists("write_json_safe")) {

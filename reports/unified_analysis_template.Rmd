...
tryCatch({
  ...
  cat('<div style="border: 2px solid #2196F3; padding: 15px; border-radius: 8px; background: #F5F9FF; margin: 10px 0 20px 0;">\n')
  cat('<strong style="font-size: 1.1em;">Executive Summary</strong><br><br>\n')

  cat(sprintf("<strong>Assay:</strong> %s &mdash; %s<br>\n", exec_assay_type, exec_analyte))
  cat(sprintf("<strong>Analysis date:</strong> %s<br>\n", Sys.Date()))
  ...
  cat(sprintf("<strong>Standards:</strong> %d concentration levels", n_std_concs))
  if (n_hv > 0) cat(sprintf(" (%d excluded for high CV)", n_hv))
  cat("<br>\n")

  # Sample count
  n_samples <- sum(data_long$SampleType == "Sample", na.rm = TRUE)
  cat(sprintf("<strong>Sample wells:</strong> %d<br>\n", n_samples))

  # Note: Model fit details (R2, LLOQ/ULOQ, status) are populated after model fitting
  # in the exec-summary-box chunk below.
  cat('<em>Model results and overall status appear after curve fitting below.</em><br>\n')
...
<details>
<summary>Click to expand standard concentrations</summary>
...
cat('\n<details>\n<summary>Click to expand QC traffic light summary</summary>\n\n')
...
if (multi_weight_mode && length(all_models) > 1) {
  cat('\n<details>\n<summary>Click to expand weighting comparison</summary>\n\n')
  ...
  if (knitr::is_html_output()) {
    print(htmltools::tagList(plotly::ggplotly(cmp_plot)))
  } else {
    print(cmp_plot)
  }
...
tryCatch({
  if (!drc_failed_completely && length(all_models) > 0) {
    ...
    cat('\n<details>\n<summary>Click to expand model stability assessment</summary>\n\n')
    cat("\n\n<h4>Model Stability Assessment</h4>\n\n")
...
tryCatch({
  cat('\n<details>\n<summary>Click to expand LLOQ/ULOQ determination</summary>\n\n')
...
<details>
<summary>Click to expand dose-response curve</summary>
...
<details>
<summary>Click to expand 4-PL model coefficients</summary>
...
<details>
<summary>Click to expand standard back-calculation and recovery</summary>
...
<details>
<summary>Click to expand sample results summary</summary>
...
if (isTRUE(analysis_config$enable_outlier_detection) && exists("outlier_flags") && nrow(outlier_flags) > 0) {
  cat('\n<details>\n<summary>Click to expand outlier detection results</summary>\n\n')
...
# Close the outlier details block (opened in outlier-results chunk)
if (isTRUE(analysis_config$enable_outlier_detection)) {
  cat('\n</details>\n\n')
}
```
...
if (exists("sample_results") && !is.null(sample_results) && nrow(sample_results) > 0) {
  ...
  if (nrow(sample_plot_data) > 0) {
    ...
    # Render plot
    if (knitr::is_html_output()) {
      p <- plotly::ggplotly(sample_box, tooltip = "text", height = fig_h * 96) %>%
        plotly::layout(margin = list(b = 120, l = 80))
      cat('<details>\n<summary>Click to expand sample variability visualization</summary>\n')
...
cat('\n<details>\n<summary>Click to expand plate positional quality</summary>\n\n')
cat("\n\n<h4>Plate Positional Quality</h4>\n\n")
...
<details>
<summary>Click to expand dose-response curve with sample estimates</summary>
...
## Parallelism and Relative Potency
...
tryCatch({
  if (exists("model_fit") && !is.null(model_fit) && exists("data_long") && exists("response_var")) {
    ...
    if (isTRUE(parallelism_result$applicable)) {
      ...
      cat(sprintf("*Not applicable:* %s\n\n", parallelism_result$reason))
    }
  } else {
    cat("*Not applicable:* No fitted model available for parallelism assessment.\n\n")
  }
}, error = function(e) {
...
tryCatch({
  if (isTRUE(is_elisa) && exists("tissue_weights") && !is.null(tissue_weights) && length(tissue_weights) > 0) {
    cat('\n<details>\n<summary>Click to expand tissue normalization traceability</summary>\n\n')
    cat("\n\n<h4>Tissue Normalization Method</h4>\n\n")
...
<details>
<summary>Click to expand exclusion audit trail</summary>

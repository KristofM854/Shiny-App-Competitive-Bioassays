...
i18n_path <- file.path(script_dir, "i18n.R")
...
if (isTRUE(is_elisa)) {
  ...
  if (!is.null(control_summary)) {
    cat("**Control Well QC:**\n")
    cat("- Blank average:", round(control_summary$blank_avg, 3), "\n")
    cat("- NSB average:", round(control_summary$nsb_avg, 3), "\n") 
    cat("- B0 average:", round(control_summary$b0_avg, 3), "\n")
...
# `r if (isTRUE(is_elisa)) "ELISA" else "RBA"` Analysis Report
...
if (isTRUE(is_elisa)) {
  cat("This report analyzes ELISA data using a four-parameter logistic dose–response curve to estimate concentrations of", 
      assay_config$analyte %||% "analyte", "in unknown samples.\n\n")
  cat("**Method:** Competitive enzyme-linked immunosorbent assay with %B/B0 normalization\n\n")
} else {
  cat("This report analyzes Receptor Binding Assay (RBA) data using a four-parameter logistic dose–response curve to estimate concentrations of",
      assay_config$toxin_standard_label %||% "biotoxin", "in unknown samples.\n\n")
  cat("**Method:** Competitive receptor binding assay with", assay_config$detection_method %||% "radioligand", "detection\n\n")
}
```
...
if (nzchar(trimws(notes_text))) {
  ...
  cat("*No additional notes provided.*")
}
```
...
render_table(
  standard_table, 
  caption = "Standard Concentrations",
...
drc_plot <- drc_plot + 
  ylab(labels$y_label) +
  ggtitle(paste("Four-Parameter Logistic Model -", toupper(assay_config$assay_type), "Analysis"))
...
tryCatch({
  ...
  if (is.matrix(coefficients) && nrow(coefficients) >= 4) {
    ...
    render_table(
      drc_results,
      caption = "Four-Parameter Logistic Model Coefficients",
...
- **Standards Used** = `r nrow(standards_for_model)` of `r length(unique(standards_data$StandardConc))`
...
### Standard Back-Calculation and Recovery

```{r standard-backcalculation}
...
render_table(recovery_display, 
            caption = "Standard Back-Calculation and Recovery")

# Report overall recovery statistics
mean_overall_recovery <- mean(recovery_summary$mean_recovery, na.rm = TRUE)
cat(sprintf("\n**Overall Mean Recovery:** %.1f%%\n", mean_overall_recovery))
if (mean_overall_recovery >= 80 && mean_overall_recovery <= 120) {
  cat("✅ Overall recovery is within acceptable range (80–120%).\n\n")
} else {
  cat("⚠️ Overall recovery is outside the typical acceptable range (80–120%). Review curve fit.\n\n")
}
```
...
tryCatch({
  ...
  if (exists("replicate_stats") && !is.null(replicate_stats) && nrow(replicate_stats) > 0) {
    ...
    caption_text <- paste("Sample Quantification Results -", toupper(assay_config$assay_type))
    if (isTRUE(is_elisa) && isTRUE(has_tissue_weights)) {
      caption_text <- paste(caption_text, "(with tissue normalization)")
    }
    ...
    cat("- `unknown_results.csv` - Individual well results\n") 
    cat("- `unknown_results_summary.csv` - Replicate group statistics with confidence intervals\n\n")
    ...
    if (!is.null(high_cv_groups) && nrow(high_cv_groups) > 0) {
      cat("**⚠️ Quality Alert:**\n")
      cat("- Replicate groups with high variability (CV > 30%):", 
          paste(high_cv_groups$Replicate, collapse = ", "), "\n")
      cat("- Consider checking sample preparation or dilution consistency\n\n")
    } else {
      cat("**✅ Quality Check:** All replicate groups show acceptable variability (CV ≤ 30%)\n\n")
    }
  ...
  if (nrow(unknown_plot_data) > 0) {
    ...
    combined_drc <- ggplot() +
      ...
      labs(x = labels$x_label, y = labels$y_label,
           title = "Dose-Response Curve with Standards and Unknown Samples")
...
*Automated bioassay analysis using modular system v2.0*